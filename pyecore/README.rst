...
.. code-block:: python
    ...
    >>> # This will install dependencies lxml and ordered-set
    >>> pew new testenv
...
To test these dependencies it may be preferable to create a testenv using pew

How to install pew

* Use pipsi rather than plain ip to install pew

.. code-block:: pipsi install pew

* Pew allows the user to switch between different versions of python using multiple virtual environments

* If python3 is installed by default, run the following command

.. code-block: pew new test

* If python3 is not installed by default, run the following command

..code-block: pew new -p /usr/bin/python3 testenv
